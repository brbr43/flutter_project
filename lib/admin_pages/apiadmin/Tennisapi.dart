...
class Tennisapi {
  ...
  Future<List<Tennisbooking>> fetchBookings(String token) async {
    print("Token: $token");
    try {
      final response = await _dio.get(
        'http://10.0.2.2:8000/api/admin/tennis-bookings',
      ...
      if (response.statusCode == 200) {
        ...
        return data.map((json) => Tennisbooking.fromJson(json)).toList();
      } else {
  ...
  Future<void> updateBooking(
      String token, int id, Tennisbooking booking) async {
    try {
      print("Token: $token");

      await setToken(token); // إعداد التوكن
      final response = await _dio.put(
        'http://10.0.2.2:8000/api/admin/tennis-booking/$id',
        data: json.encode({
  ...
  Future<void> deleteBooking(String token, int id) async {
    try {
      ...
      final response = await _dio
          .delete('http://10.0.2.2:8000/api/admin/tennis-booking/$id');
      if (response.statusCode == 200) {
